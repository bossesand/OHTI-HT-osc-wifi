...
void setup() 
  {
    Wire.begin(sda, scl);
    // Are you using an ESP8266?  If so, the syntax for the begin function is: Wire.begin(int sda, int scl);
 // Wire.begin(0, 2); //SDA, SCL - OHTI Pins configure on D1 Mini (wemos) to enable straight 4 pin connection to GY-BNO055 chinese with pin order 5V, GND, SCL, SDA
  ...
  // PIN_PULLUP_EN(5); Exception 9: LoadStoreAlignmentCause: Load or store to an unaligned address
...
/*
    ...
    */
    ...
    // OSCMessage
    OSCMessage msg("/SceneRotator/quaternions");  // IEM
    msg.add(qw);
    msg.add(qx);
    msg.add(qy);
    msg.add(qz);
