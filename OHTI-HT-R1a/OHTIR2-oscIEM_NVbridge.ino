...
// Aslo save the calibration values to program memory and load the old calibration values if pin is low.
...
// bort 20201103 uint8_t Abw = ABW_31_25Hz;        // Accel bandwidth, accel sample rate divided by ABW_divx
...
void setup()
{
  ...
  // init the D1 - GPIO05 internal pullup resistor - Bosse
  //20201104 PIN_PULLUP_EN(5);
...
void initBNO055() {
  ...
  // Configure ACC
  // - ERROR writeByte(BNO055_ADDRESS, BNO055_ACC_CONFIG, APwrMode << 5 | Abw << 3 | Ascale );
  // bort 20201103 writeByte(BNO055_ADDRESS, BNO055_ACC_CONFIG, APwrMode << 5 | Abw << 2 | Ascale );
  // Configure GYR
...
void loop() {
 ...
 /* 
   setQuatComboHex();  // The 4 raw quaternion registers as 16 hex values
   Serial.println(htQuatCombo.hex); 
 */


   /* Serial. println("Eulers:");
    Serial. println(euler0, 3); 
  Serial. println(euler1, 3);
  Serial. println(euler2, 3);

  OSCMessage msg("/rotator/ypr");  // SPARTA
  msg.add(yaw);
  msg.add(pitch);
  msg.add(roll);
...
*/
  OSCMessage msg2("/SceneRotator/quaternions");  // IEM osc message sent as UDP broadcast to address 255.255.255.255
  msg2.add(qw);
  msg2.add(qx);
  msg2.add(qy);
  msg2.add(qz);
  ...
  /*
    OSCBundle bndl;
    bndl.add("/SceneRotator/quaternions").add(qw).add(qx).add(qy).add(qz); // IEM
    bndl.add("/rotator/ypr").add(yaw).add(pitch).add(roll);  // SPARTA
    Udp.beginPacket(outIp, outPort);
  ...
  */
  ...
  // delay(500);  // 0.5 sec delay for  slow message rate test
}
